...
public class PatchWrapper implements IWrapper<Patch> {
    ...
    public PatchWrapper(PersistablePatch patch) {
        this.persistedPatch = patch;
    }
    ...
    public boolean load(Patch wrapped) {
        ...
        PolylineWrapper polyline = new PolylineWrapper(persistedPatch.getBorder());
        polyline.load(wrapped.getBorder());
    ...
    public boolean save(Patch wrapped) {
        ...
        // First, save the polyline
        PersistablePolyline polyline = persistedPatch.getBorder();
        if (polyline == null) {
            polyline = new PersistablePolyline();
            persistedPatch.setBorder(polyline);
        }
        PolylineWrapper polylineWrapper = new PolylineWrapper(polyline);
        polylineWrapper.save(wrapped.getBorder());
        ...
        else persistedPatch.setFaciesId(-1);
            ...
            if (!(interval instanceof ContactInterval)) {
                ...
                if (index != -1) {
                    ...
                    for (ICurviPoint point : wrapped.getBorder().getPoints()) {
                        if (point.getCurvilinear() == s1s2[index * 2]) s1 = point;

                        if (point.getCurvilinear() == s1s2[index * 2 + 1]) s2 = point;
                    }
