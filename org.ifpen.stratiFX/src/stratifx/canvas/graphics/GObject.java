 ...
 * geometry (GSegments) or in terms of sub objects (or both).
 * <p>
...
public class GObject
    ...
    /**
     * Return the region of this graphic object. The region is a union of all
     * children objects and the geometry of this object.
     *
    ...
    /**
     * Add a sub object to this graphics object. Use as:
     *
     * <ul>
    ...
    /**
     * Move this object to the front of its siblings in its parent object. If
     * doesn't have a parent, this method has no effect. This is a convenience
     * shorthand of getParent().reposition (this, getParent().front());
     */
    public void toFront() {
        if (parent_ != null) {
    ...
    /**
     * Move this object to the behind its siblings in its parent object. If
     * doesn't have a parent, this method has no effect. This is a convenience
     * shorthand of getParent().reposition (this, getParent().back());
     */
    public void toBack() {
        if (parent_ != null) {
    ...
    /**
     * Return the position code for the position behind the specified child
     * node.
     *
    ...
    /**
     * Return the position code for the position in front of the specified child
     * node.
     *
     * @see #reposition(GObject,int)
     *
    ...
    /**
     * Return the position of the specified child among the children of this
     * GObject.
     *
     * @param child Child to find index of.
     * @return Index of child (or -1 if no such child). 0 indicates back, and so
     * on.
     */
    ...
    /**
     * Find a child object with specified name. Search entire sub tree from this
     * node, depth first.
     *
    ...
    /**
     * Return all segments that intersects the specified rectangle. Serach this
     ...
     * @return List of segments intersecting the rectangle. If none do, an empty
     * list is returned.
     */
     ...
     * @param x1 X coordinate of lower right corner of rectangle.
     * @param y1 Y coordinate of lower right corner of rectangle.
     * @return A segment intersecting the rectangle (or null if none do).
     */
    ...
    /**
     * Return the first segment found that are completely inside the specified
     * rectangle. Search this node, and all sub nodes.
     *
     * @param x0 X coordinate of upper left corner of rectangle.
     * @param y0 Y coordinate of upper left corner of rectangle.
     * @param x1 X coordinate of lower right corner of rectangle.
    ...
    /**
     * Return all segments that intersects with the specified point. Search this
     * node, and all sub nodes.
     *
     * @param x, y Point to check.
     * @return All segments intersecting the point. If none do, an empty list is
     * returned.
     */
    ...
    /**
     * Find all segments of the subtree rooted at this GObject that intersects
    ...
    private void findSegments(int x0, int y0, int x1, int y1, List segments) {
        ...
        if (!region_.isIntersecting(new GRect(x0, y0, x1 - x0 + 1, y1 - y0 + 1))) {
            return;
        }
    ...
    /**
     * Find all segments of the subtree rooted at this GObject that are inside
     * the specified rectangle.
     *
     * @param x0 X coordinate of upper left corner of rectangle.
     * @param y0 Y coordinate of upper left corner of rectangle.
     ...
     * @param x1 X coordinate of lower right corner of rectangle.
     * @param y1 Y coordinate of lower right corner of rectangle.
     * @return Front-most object intersecting the specified rectangle (or null
     * if none do).
     */
     ...
     * @param x1 X coordinate of lower right corner of rectangle.
     * @param y1 Y coordinate of lower right corner of rectangle.
     * @return Front most child object that are fully inside the specified
     * rectangle (or null if none are).
     */
     ...
     * @param x X coordinate of point to check.
     * @param y Y coordinate of point to check.
     * @return Front-most object intersecting the point (or null if none do).
     */
    ...
    /**
     * Find all objects intersecting a specified rectangle. Objects are returned
     * with front most object on screen last in list. Seacrh is done in the
     * subtree of this object (including this). If no objects intersects, an
    ...
    /**
     * Find all objects inside a specified rectangle. Objects are returned with
     * front most object on screen last in list. Seacrh is done in the subtree
     * of this object (including this). If no objects is inside, an empty list
     * is returned.
     *
    ...
    /**
     * Find all objects intersecting a specified point. Objects are returned
     * with front most object on screen last in list. Seacrh is done in the
     * subtree of this object (including this). If no objects intersects, an
     * empty list is returned.
     *
    ...
    private void findAllInside(int x0, int y0, int x1, int y1, List objects) {
        GBox box = new GBox(x0, y0, x1, y1);

        // Don't scan any furher if the region doesn't intersect
        if (!region_.isIntersecting(new GRect(box))) {
            return;
        }
        ...
        if (region_.isInsideOf(new GRect(box))) {
            objects.add(this);
        }
    }
     ...
     *
     * @return The sibling object in the immediate front of this object (or null
     * if this object is in front, or it is not attach to a parent).
     */
     ...
     *
     * @return The sibling object immediately behind this object (or null if
     * this object is in back, or it is not attach to a parent).
     */
     ...
     *
     * @param segment Segment to remove. If the specified segment is not a child
     * of this GObject, this call has no effect.
     */
    ...
    /**
     * Remove sequence of segments from this object. Remove all from specified
     * start segment.
     *
     * @param from Starting segment
     */
    ...
    void refreshData(int visibilityMask, GRegion damagedRegion) {
        ...
        if (segments_ != null) {
            ...
            for (Iterator i = segments_.iterator(); i.hasNext();) {
                ...
                if ((visibilityMask & SYMBOLS_VISIBLE) == SYMBOLS_VISIBLE) {
    ...
    /**
     * The redraw method ensures that the geometry in the entire scene becomes
     * up to date. Called initially and then in the following cases:
     *
     * <ul>
     * <li>resize
     * <li>setViewport
     * <li>setWorldExtent
     * <li>setVisibility to on, because invisible views are not redrawn in the
     * case above.
     * </ul>
     *
     ...
     * <p>
     * Normally this method is called automatically whenever needed (typically
     * on retransformations). A client application <em>may</em> call this method
     * explicitly if some external factor that influence the graphics has been
     * changed. However, beware of the performance overhead of such an approach,
     * and consider reorganizing the code so that model changes explictly
     * affects the graphics elements.
     */
    public void redraw() {
    ...
    /**
     * Set new style for this object. Style elements not explicitly set within
     * this GStyle object are inherited from parent objects. Child objects with
     * not explicit set style will inherit from this style object. A GObject
     * does not need a style instance; in this case all style elements are
     * inherited.
     *
     * @param style Style of this object (or null if the intent is to unset the
     * current style).
     */
    ...
    /**
     * Return style of this object. This is the style set by setStyle() and not
     * necesserily the style as it appears on screen as unset style elements are
     * inherited from parents.
     *
     * @return Style of this object as specified with setStyle(), (or null if no
    ...
    /**
     * This is the actual style used for this object when inheritance for unset
     * values are resolved.
     *
    ...
    /**
     * Resolve unset values in the style element of this object. This involved
     * creating a default "actual style" element, override with style elements
     * of the parents actual style and override with style elements explicitly
     * set in the style of this object (if any).
     */
     ...
     * <p>
     * NOTE: The present visibility is maintained, and the new flag is taken as
     * an <em>instruction</em> for change.
     *
     ...
     * </ul>
     *
     * The settings VISIBLE and INVISIBLE are conveniences compound for all
     * visible and invisible settings respectively.
     *
    ...
    public final void setVisibility(int visibilityMask) {
        ...
        if ((oldVisibilityMask & DATA_VISIBLE)
            ...
            if ((visibilityMask & DATA_VISIBLE) != 0) {
                getScene().computeRegion(visibilityMask);
            }
    ...
    /**
     * Compute position of all GImage in the subtree rooted at this GObject.
     */
    void computeSymbolPositions() {
        GScene scene = getScene();
        if (scene == null) {
            return;
        }
        ...
        if ((visibilityMask_ & SYMBOLS_VISIBLE) == 0) {
            return;
        }
        ...
        if (segments_ != null) {
            for (Iterator i = segments_.iterator(); i.hasNext();) {
                GSegment segment = (GSegment) i.next();
                GImage image = segment.getVertexImage();
                if (null != image) {
                    scene.computeVertexPositions(image);
                }
            }
        }
    }
    ...
    /**
     * Compute position of all GTexts in the subtree rooted at this GObject.
     */
