...
class GAnnotator
{
   ...
   * position as dictated by its rendering hint and before any adjustments
   * are done.
   * 
                                     ...
                                     int pointNo)
  {
    ...
    if ( (positionHint & GPosition.OFFSET) != 0 ){
    	pointNo += positional.getPositionOffset(); 
    	if ( pointNo > x.length ){
    		pointNo -= x.length;
    	}
    }
  ...
  /**
   * Adjust the position of the specified positional according to
   * point position hint.
   * 
                                           ...
                                           int x[], int y[], int pointNo)
  {
    ...
    if (distance_ == 0.0) {
      ...
      double legLength = Vector2D.length (x[pointNo], y[pointNo],
    ...
    for (int i = 0; i < pointNo; i++)
      totalLength += Vector2D.length (x[i], y[i], x[i+1], y[i+1]);
    ...
    GRect  leftRectangle  = new GRect (positional.getRectangle());
    GRect  rightRectangle = new GRect (positional.getRectangle());
                                ...
                                int x[], int y[], int pointNo)
  {
    ...
    if (intersection == null)
      ...
      distance_ = Vector2D.length (intersection[0], intersection[1],
                                   x[iPoint], y[iPoint]);
    }
  ...
  private int[] findViewportIntersection (int x0, int y0, int x1, int y1)
  {
    ...
    if (type == 1l) {
      ...
      length[n] = Vector2D.length (x0, y0, x[n], y[n]);
      n++;
    }
